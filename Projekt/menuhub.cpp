...
void RulesMenu::setPagesContent()
{
	...
	{
		{ "GAME RULES", {
		"THE OBJECTIVE OF THE GAME IS TO SURVIVE AS MANY WAVES OF ENEMIES AS POSSIBLE AND ACHIEVE THE HIGHEST SCORE.",
		"BEFORE STARTING THE GAME, YOU CHOOSE ONE OF THE AVAILABLE CHARACTERS. EACH CHARACTER HAVE DIFFERENT SET OF STATS.",
		"THE INITIAL WAVE CONTAINS 5 OPPONENTS, AND EACH SUBSEQUENT WAVE INCREASES IN DIFFICULTY AND NUMBER OF ENEMIES."}},

		{"GAME RULES", {
		"WHEN YOU DEFEAT AN ENEMY, THERE IS A CHANCE THEY WILL DROP AN ITEM. YOU CAN PICK IT UP BY STEPPING ON IT.", 
		"IF YOU LOSE HEALTH, THE ONLY WAY TO RESTORE IT IS BY PICKING UP A HEART CONTAINER DROPPED BY AN ENEMY.", 
		"ALL NON-COLLECTIBLE ITEMS DISAPPEAR AT THE START OF THE NEXT WAVE - UNLESS THE UPCOMING WAVE IS A BOSS FIGHT."}},

		{"GAME RULES", {
		"EVERY 5 WAVES, YOU WILL FACE A BOSS. DEFEATING THE BOSS GRANTS YOU SPECIAL COLLECTIBLE ITEM.",
		"YOU CAN UNLOCK ITEMS BY COMPLETING THE QUESTS, WHICH ARE DISPLAYED BOTH ON ITEM'S DESCRIPTION PAGE AND IN YOUR COLLECTION.",
		"ALL CHARACTER, ENEMY AND ITEM STATS ARE LISTED ON THE FOLLOWING PAGES."}}

	};
	CharInfoPages =
	{
		{"CHARACTERS", "SPIRIT OF BALANCE", { "CHARACTER STATS ARE SHOWN BELOW:" }, "FirstCharacter.png", 0},
		{"CHARACTERS", "MISTCALLER", { "CHARACTER STATS ARE SHOWN BELOW:" }, "SecondCharacter.png", -1},
		{"CHARACTERS", "FANGSHADE", { "CHARACTER STATS ARE SHOWN BELOW:" }, "ThirdCharacter.png", 1},
	};
	...
	EnemyInfoPages = 
	{
		...
		{"BOSSES", "RAGING GIANT", "Monster5", { "THIS BOSS CHASES YOU AND RAPIDLY FIRES TEARS.","THEIR LARGE HITBOX MAKES THEM HARD TO MISS." }, "Enemy5.png",3.25},
		{"BOSSES", "WITHERING ROOT", "Monster6", { "THIS BOSS TELEPORTS IN FRONT OF YOU EVERY FEW SECONDS, WHILE ALSO SHOOTING TEARS.", "THEY HAVE HUGE AMOUNT OF HEALTH."}, "Enemy6.png",1.75},
...
void RulesMenu::DrawRules(int page)
{
	...
	float smallestFont = 40 * ScreenSettings::GetInstance().getScreenResolutionFactor().y;
	...
	if (page > CharInfoLimit && page <= EnemyInfoLimit)
	{
		...
		GameUI::GetInstance().DrawEnemyStatsInMenu(EnemyPage.enemyName, paragraphsWithImagesBar, smallestFont, y_pos);
	}
